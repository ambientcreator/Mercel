...
// RU: РўРµСЃС‚ `TestExistingFreshMercelDatabaseIsRecoveredFromLegacy`.
...
func TestExistingFreshMercelDatabaseIsRecoveredFromLegacy(t *testing.T) {
	...
	for idx := 0; idx < 8; idx++ {
		...
		if _, err := newDB.Exec(`INSERT INTO services(code, name, unit, rate, category, allocation_percent, created_by, created_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?)`, code, code, "С‡.", 100, CategoryPrimary, nil, "admin", "2026-03-28T00:56:47+03:00"); err != nil {
...
// RU: РўРµСЃС‚ `TestBuildServiceTargetsDistributesDefaultsEvenly`.
...
// RU: РўРµСЃС‚ `TestBuildServiceTargetsWeightAddsShareWithinGroup`.
...
// RU: РўРµСЃС‚ `TestSeniorSpecialistCanDeleteEmployeeOnly`.
